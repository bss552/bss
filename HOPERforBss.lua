...
local TOKEN = "MTE0NTAzNTYwNDU3MzMwNjk2MQ.GQpbyP.xYHnnMaiGy6neHma2wQDa1m4Th5gOcdHeUj3mQ" 
local CHANNEL_ID = "1405286175677222973" -- Ростки
local DISCORD_TOKEN = "MTE0NTAzNTYwNDU3MzMwNjk2MQ.GQpbyP.xYHnnMaiGy6neHma2wQDa1m4Th5gOcdHeUj3mQ" 
local TARGET_CHANNEL = "1405264607399907468" -- Вициусы
...
local function checkChannel(channel, auth)
    ...
    if res and res.StatusCode == 200 then
        ...
        if data[1] then
            ...
            return extractJobId(content), msg.id
        end
    end
    return nil, nil
end
...
task.spawn(function()
    while true do
        if canJump and not isLooting and not findMonster("Vicious Bee") and not findMonster("Stick Bug") then
            ...
            if stid and stmid ~= lastST and stid ~= game.JobId and not table.find(blacklist, stid) then
                targetId = stid
                targetType = "STICK"
                writefile("last_st_msg.txt", stmid)
            end

            -- 2. Средний приоритет: Ростки (только если не найден Стик)
            if not targetId then
                local sid, smid = checkChannel(CHANNEL_ID, TOKEN)
                if sid and sid ~= game.JobId and not table.find(blacklist, sid) then
                    targetId = sid
                    targetType = "SPROUT"
                end
            end
            ...
            if not targetId then
                ...
                if bid and bmid ~= lastV and bid ~= game.JobId and not table.find(blacklist, bid) then
                    targetId = bid
                    targetType = "BEE"
                    writefile("last_v_msg.txt", bmid)
                end
            end
            ...
            if targetId then
                ...
                table.insert(blacklist, targetId)
